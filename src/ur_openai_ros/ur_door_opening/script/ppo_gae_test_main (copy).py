...
def main():
    ...
    success_list = deque(maxlen=maxlen_num) # 10
    fail_knob_and_panel_list = deque(maxlen=maxlen_num) # 10
    fail_taxle_list = deque(maxlen=maxlen_num) # 10
    fail_force_list = deque(maxlen=maxlen_num) # 10
    fail_torque_list = deque(maxlen=maxlen_num) # 10
    ...
    ax2.hist(arr_x_data_success_hist, label="success")
    ax2.hist(arr_x_data_fail_hist, label="fail")
    ax2.set_xlabel("x")
    ax2.set_ylabel("success&fail")
    ax2.grid(axis='both')
    ax2.legend(loc=2)
    ax3 = fig.add_subplot(4, 3, 3)
    ax3.hist(arr_y_data_success_hist, label="success")
    ax3.hist(arr_y_data_fail_hist, label="fail")
    ax3.set_xlabel("y")
    ax3.set_ylabel("success&fail")
    ax3.grid(axis='both')
    ax3.legend(loc=2)
    ...
    ax11.plot(x_data_f, y_data_success, 'r-', linestyle="solid", label="success")
    ax11.set_xlabel("episodes")
    ax11.set_ylabel("success")
    ax11.grid(axis='both')
    ax11.legend(loc=3)
    ax12 = fig.add_subplot(4, 3, 12)
    ax12.plot(x_data_f, y_data_fail_knob_and_panel, 'r-', linestyle="solid", label="knob_or_panel")
    ax12.plot(x_data_f, y_data_fail_taxel, 'b-', linestyle="solid", label="taxel")
    ax12.plot(x_data_f, y_data_fail_force, 'g-', linestyle="solid", label="force")
    ax12.plot(x_data_f, y_data_fail_torque, 'm-', linestyle="solid", label="torque")
    ax12.set_xlabel("episodes")
    ax12.set_ylabel("failure_mode")
    ax12.grid(axis='both')
    ax12.legend(loc=3)
...
#        fail_knob_and_panel = update - env.success_times - env.fail_taxel_limit - env.fail_force_limit - env.fail_torque_limit
#        fail_knob_and_panel_list.append(fail_knob_and_panel)
#        fail_taxle_list.append(env.fail_taxel_limit)
#        fail_force_list.append(env.fail_force_limit)
#        fail_torque_list.append(env.fail_torque_limit)

        if (update%1) == 0:
            print('[{}/{}]'.format(env.success_times, update))

#            print('[{}/{}] fail_knob&panel : {}, fail_taxel : {}, fail_force : {}, fail_torque : {}'.format(
#                env.success_times, update, fail_knob_and_panel, env.fail_taxel_limit, env.fail_force_limit, env.fail_torque_limit))
...
#        y_data_fail_knob_and_panel.append(np.mean(fail_knob_and_panel_list))
#        y_data_fail_taxel.append(np.mean(fail_taxle_list))
#        y_data_fail_force.append(np.mean(fail_force_list))
#        y_data_fail_torque.append(np.mean(fail_torque_list))
...
#            ax12.plot(x_data_f, y_data_fail_torque, 'm-', linestyle="solid")
...
#        if (np.mean(avg_max_knob_rotation_list) > 0.8 and np.mean(avg_max_door_rotation_list) > 4): # Threshold return to success 
