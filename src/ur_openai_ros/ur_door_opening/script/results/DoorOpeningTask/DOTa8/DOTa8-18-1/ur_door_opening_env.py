...
#        print("ee_rpy_ini", self.eef_rpy_ini) # x: -0.022194749153057504 y: 0.9997526460232887 z: -0.0011061239414521327
    ...
    def step(self, action, update):
        ...
        for x in range(1, sub_step + 1):
            ...
            if self.moveit == 0:
                sub_action[0] = sub_action[0] / sub_a0
                sub_action[1] = sub_action[1] / sub_a1
                sub_action[2] = sub_action[2] / sub_a2
                sub_action[3] = sub_action[3] / sub_a3
                sub_action[4] = sub_action[4] / sub_a4
                sub_action[5] = sub_action[5] / sub_a5
...
#        print("door_rotation, act, ini", self.door_rotation, self.door_rpy, self.door_rpy_ini)
        ...
        if self.knob_rotation < 0.2:
            ...
            self.knob_rotation_r = self.knob_rotation * knob_c + knob_bonus_c * 3 # 0.9 * 100 + 10 * 3 = 120 (90-120)
#        elif 0.9 < self.knob_rotation < 1.0:
#            self.knob_rotation_r = self.knob_rotation * knob_c + knob_bonus_c * 4  # 1 * 100 + 10 * 4 = 140 (130-140)
#            self.panel_rotation_r =  self.door_rotation * panel_c
        elif 0.9 < self.knob_rotation:
            self.knob_rotation_r = 1.0 * knob_c + knob_bonus_c * 14 # 1 * 100 + 10 * 14 = 240
            if self.door_rotation < 0.2:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c     # + 0.2 * 100 + 10 (0-30)
            elif 0.2 < self.door_rotation < 0.4:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 2 # + 0.4 * 100 + 10 * 2 (40-60)
            elif 0.4 < self.door_rotation < 0.6:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 3 # + 0.6 * 100 + 10 * 3 (70-90)
            elif 0.6 < self.door_rotation < 0.8:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 4 # + 0.8 * 100 + 10 * 4 (100-120)
            elif 0.8 < self.door_rotation < 1.0:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 5 # + 1.0 * 100 + 10 * 5 (130-150)
            elif 1.0 < self.door_rotation:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 10 #+ 1.0 * 100 + 10 *10 (200-)
