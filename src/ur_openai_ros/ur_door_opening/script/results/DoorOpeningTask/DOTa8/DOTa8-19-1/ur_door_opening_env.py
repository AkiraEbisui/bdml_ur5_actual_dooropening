...
#        print("door_rotation, act, ini", self.door_rotation, self.door_rpy, self.door_rpy_ini)
        ...
        #2 door panel open(+), 
        if self.knob_rotation < 0.12:
            self.knob_rotation_r = self.knob_rotation * knob_c                     # 0.12 * 100 = 12 (0-12)
            print("knob_rotation < 0.12")
        elif 0.12 <= self.knob_rotation < 0.24:
            self.knob_rotation_r = self.knob_rotation * knob_c + knob_bonus_c      # 0.24 * 100 + 10 = 34 (22-34)
            print("0.12 < knob_rotation < 0.24")
        elif 0.24 <= self.knob_rotation < 0.36:
            self.knob_rotation_r = self.knob_rotation * knob_c + knob_bonus_c * 2  # 0.36 * 100 + 10 * 2 = 56 (44-56)
            print("0.24 < knob_rotation < 0.36")
        elif 0.36 <= self.knob_rotation < 0.53:
            self.knob_rotation_r = self.knob_rotation * knob_c + knob_bonus_c * 5  # 0.53 * 100 + 10 * 5 = 103 (86-103)
            print("0.36 < knob_rotation < 0.53")
            if self.door_rotation < 0.12:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c      # 0.12 * 100 + 10 (0-22)
                print("panel_rotation < 0.12")
            elif 0.12 <= self.door_rotation < 0.24:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 2  # 0.24 * 100 + 10 * 2 (32-44)
                print("0.12 < panel_rotation < 0.24")
            elif 0.24 <= self.door_rotation < 0.36:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 3  # 0.36 * 100 + 10 * 3 (54-66)
                print("0.24 < panel_rotation < 0.36")
            elif 0.36 <= self.door_rotation < 0.89:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c * 5  # 0.48 * 100 + 10 * 5 (86-98)
                print("0.36 < panel_rotation < 0.89")
            elif 0.89 <= self.door_rotation:
                self.panel_rotation_r =  0.89 * panel_c + panel_b_c * 10 # 0.89 * 100 + 10 * 10 (189- )
                print("0.89 < panel_rotation")
        elif 0.53 <= self.knob_rotation:
            self.knob_rotation_r = 0.53 * knob_c + knob_bonus_c * 5 # 0.53 * 100 + 10 * 5 = 103 (103- )
            print("0.53 < knob_rotation")
            if self.door_rotation < 0.12:
                self.panel_rotation_r =  self.door_rotation * panel_c + panel_b_c      # 0.12 * 100 + 10 (0-22)
                print("panel_rotation < 0.12")
            elif 0.12 <= self.door_rotation < 0.24:
